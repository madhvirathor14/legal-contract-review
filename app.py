...
.ring-score {
  ...
  color: var(--gold); line-height: 1; transition: all 0.3s;
}
...
.task-top { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 12px; }
...
.task-name { font-family: 'JetBrains Mono', monospace; font-size: 0.78rem; font-weight: 500; color: var(--text); margin-bottom: 8px; }
.task-desc { font-size: 0.82rem; color: var(--muted); line-height: 1.6; margin-bottom: 16px; }
.task-meta { display: flex; gap: 16px; font-family: 'JetBrains Mono', monospace; font-size: 0.65rem; color: var(--muted); }
...
main {
  ...
  max-width: 1200px; margin: 0 auto;
  padding: 0 40px 40px; margin-top: 32px;
}
...
.panel-sub { font-family: 'JetBrains Mono', monospace; font-size: 0.65rem; color: var(--muted); }
...
.btn-gold { background: linear-gradient(135deg, var(--gold), var(--gold2)); color: #0a0a0a; box-shadow: 0 4px 16px rgba(201,168,76,0.2); }
.btn-gold:hover { box-shadow: 0 4px 24px rgba(201,168,76,0.35); }
.btn-green { background: var(--green-dim); color: var(--green); border: 1px solid rgba(61,220,132,0.3); }
...
.btn-sm { font-size: 0.65rem; padding: 5px 12px; width: auto; }
.btn-gap { height: 10px; }
...
.response-box {
  ...
  white-space: pre-wrap; word-break: break-all; transition: border-color 0.3s;
}
...
.log-scroll { max-height: 220px; overflow-y: auto; display: flex; flex-direction: column; gap: 4px; }
...
.log-entry {
  ...
  background: var(--surface2); animation: logSlide 0.3s ease;
}
...
.log-tag {
  ...
  padding: 1px 6px; border-radius: 3px; flex-shrink: 0; margin-top: 1px;
}
...
.log-msg { font-family: 'JetBrains Mono', monospace; font-size: 0.68rem; color: var(--muted); line-height: 1.5; }

.stats-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 16px; }

.stat-box { background: var(--surface2); border: 1px solid var(--border); border-radius: 8px; padding: 12px 14px; text-align: center; }
.stat-val { font-family: 'Cormorant Garamond', serif; font-size: 1.8rem; font-weight: 600; color: var(--gold); line-height: 1; transition: all 0.3s; display: block; }
.stat-lbl { font-family: 'JetBrains Mono', monospace; font-size: 0.6rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.08em; margin-top: 4px; display: block; }
...
.method-tag {
  font-family: 'JetBrains Mono', monospace;
  font-size: 0.62rem; font-weight: 500;
  padding: 2px 8px; border-radius: 3px; flex-shrink: 0; min-width: 46px; text-align: center;
}
...
.ep-path { font-family: 'JetBrains Mono', monospace; font-size: 0.78rem; color: var(--text); flex: 1; }
.ep-desc { font-size: 0.75rem; color: var(--muted); flex: 1.5; }

.reward-bar-wrap { margin: 12px 0; background: var(--surface2); border: 1px solid var(--border); border-radius: 6px; overflow: hidden; height: 6px; }
.reward-bar { height: 100%; background: linear-gradient(90deg, var(--gold), var(--gold2)); width: 0%; transition: width 0.5s ease; border-radius: 6px; box-shadow: 0 0 8px rgba(201,168,76,0.4); }
...
<section class="hero">
  ...
  <div class="hero-grid">
    <div class="hero-content">
      <h2 class="hero-title">AI Agents That<br>Think Like <em>Lawyers</em></h2>
      <p class="hero-desc">
      ...
      <div class="hero-stats">
        <div><span class="hero-stat-val">3</span><div class="hero-stat-lbl">Tasks</div></div>
        <div><span class="hero-stat-val">9</span><div class="hero-stat-lbl">Contracts</div></div>
        <div><span class="hero-stat-val">0–1</span><div class="hero-stat-lbl">Reward Range</div></div>
        <div><span class="hero-stat-val">∂</span><div class="hero-stat-lbl">Partial Credit</div></div>
      </div>
    </div>
...
<section style="padding: 40px; max-width: 1200px; margin: 0 auto; border-bottom: 1px solid var(--border);">
  ...
  <div class="task-cards">
    <div class="task-card easy" onclick="selectTask('find_missing_clauses')">
      ...
      <div class="task-top"><div class="task-icon">🔍</div><span class="diff-badge">Easy · 3 steps</span></div>
      ...
      <div class="task-meta"><span>🎯 Partial credit</span><span>📋 8 clause types</span></div>
    </div>
    <div class="task-card medium" onclick="selectTask('identify_risky_party')">
      <div class="task-card-glow"></div>
      <div class="task-top"><div class="task-icon">⚠️</div><span class="diff-badge">Medium · 2 steps</span></div>
      ...
      <div class="task-meta"><span>🎯 Binary + bonus</span><span>👥 4 party options</span></div>
    </div>
    <div class="task-card hard" onclick="selectTask('rewrite_ambiguous')">
      <div class="task-card-glow"></div>
      <div class="task-top"><div class="task-icon">✍️</div><span class="diff-badge">Hard · 1 step</span></div>
      ...
      <div class="task-meta"><span>🎯 5-component score</span><span>⚖️ Multi-criteria</span></div>
    </div>
  </div>
</section>
...
<main>
  <div class="panel">
    ...
    <div class="panel-body">
      ...
      <button class="btn btn-gold" onclick="doReset()">▶ POST /reset — Start New Episode</button>
      <div class="btn-gap"></div>
      <button class="btn btn-green" onclick="doStep()">⚡ POST /step — Submit Sample Action</button>
    </div>
  </div>
  ...
  <div class="panel">
    ...
    <div class="panel-body">
      <div class="stats-row">
        <div class="stat-box"><span class="stat-val" id="stat-steps">0</span><span class="stat-lbl">Steps</span></div>
        <div class="stat-box"><span class="stat-val" id="stat-score">0.00</span><span class="stat-lbl">Best Score</span></div>
        <div class="stat-box"><span class="stat-val" id="stat-done">—</span><span class="stat-lbl">Status</span></div>
      </div>
...
function updateStats(result) {
  ...
  const circ = 339.3;
  document.getElementById('score-ring').style.strokeDashoffset = circ - (score * circ);
...
async function doReset() {
  ...
  try {
    ...
    updateStats(data);
    episodeActive = true;
    addLog('RESET', 'tag-reset', `task=${currentTask} contract=${data.observation?.contract_id || '?'}`);
  } catch(e) {
...
async function doStep() {
  if (!episodeActive) { addLog('WARN', 'tag-warn', 'Call /reset first.'); return; }
  document.getElementById('last-call-label').textContent = 'POST /step';
  const sampleActions = {
    find_missing_clauses: { task: 'find_missing_clauses', find_missing: { missing_clauses: ['termination', 'governing_law', 'dispute_resolution'] } },
    identify_risky_party: { task: 'identify_risky_party', identify_risky: { clause_id: 'clause_6', disadvantaged_party: 'Party B', reasoning: 'This clause gives Client unilateral termination rights with no obligation to pay work in progress, creating severe financial exposure and disproportionate disadvantage for Party B with no recourse.' } },
    rewrite_ambiguous: { task: 'rewrite_ambiguous', rewrite: { clause_id: 'clause_1', rewritten_clause: 'Net profits shall be distributed 60% to Partner A and 40% to Partner B, calculated quarterly on net revenue by a certified public accountant within 30 days of each quarter end. Each distribution shall be made within 15 days of the accountant\'s report. Disputes shall be resolved through binding arbitration within 60 days, with costs shared equally. This obligation is binding and enforceable against both parties.' } }
  };
  try {
    const res = await fetch('/step', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(sampleActions[currentTask])
    });
    const data = await res.json();
    ...
    if (data.done) { episodeActive = false; addLog('END', 'tag-end', `Final score: ${data.observation?.score_so_far?.toFixed(3)}`); }
  } catch(e) {
    setResponse({ error: e.message }, 'error');
    addLog('ERROR', 'tag-error', e.message);
  }
...
async function doState() {
  ...
  } catch(e) { setResponse({ error: e.message }, 'error'); }
}
...
async function doHealth() {
  ...
  try {
    ...
    const data = await res.json();
    setResponse(data, 'success');
    addLog('HEALTH', 'tag-init', `status=${data.status}`);
  } catch(e) { setResponse({ error: e.message }, 'error'); }
}
